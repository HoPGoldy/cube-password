...
**前端**：⑨ `services/base.ts` 删 `requestDelete`；⑩ axios 拦截器 403 跳转改为 `window.location.href = mergeUrl(APP_CONFIG.PATH_BASENAME, "e403")`（复用 `utils/path.ts` 的 mergeUrl）；⑪ `store/lcoal.ts` 改名 `store/local.ts`（更新 import）；⑫ 删无调用方的 `pages/certificate-list/components/group-sidebar.tsx`（GroupSidebar，全仓 grep 仅定义处，T06 后已无引用方）与 `services/group.ts` 的 `useUpdateGroupSort`（无调用方）。
...
## 边界
- 允许修改：清单 ⑫ 之前各项涉及文件 + 双端 package.json + `packages/frontend/src/store/`（改名）+ 根 README.md；⑫ 的两个删除目标经 grep 确认无引用后方可执行，若发现引用则跳过并在回报中说明。
...
- `grep -rn "mockjs\|multipart\|ErrorBanned\|requireAdmin\|requestDelete\|lcoal\|cube-diary\|GroupSidebar\|useUpdateGroupSort" packages/backend/src packages/frontend/src packages/backend/package.json packages/frontend/package.json README.md packages/frontend/index.html` 无残留（README 中合理提及除外，如"从 cube-diary 分叉"说明——不需要就不写）。
