...
import { AddGroupResp, CertificateGroupDetail, CertificateGroupStorage, CertificateListItem, GroupAddPasswordData, GroupRemovePasswordData } from '@/types/group'
...
export const createGroupService = (props: Props) => {
    ...
    const updateSort = async (newSortIds: number[]) => {
        ...
        await db.knex.transaction(async trx => {
            return Promise.all(newSortIds.map(id => {
    ...
    return {
        queryGroupList, addGroup, getCertificateList, updateGroupName,
        updateSort, setDefaultGroup, deleteGroup, unlockGroup, groupAddPassword, removeGroupPassword
    }
}
